...
export class CreateinvoiceComponent implements OnInit {
    ...
    this.service.GetInvHeaderbycode(invoiceno).subscribe(res => {
      ...
      if (editdata != null) {
        this.invoiceform.setValue({
          ...
          customerName: editdata.customerName,  contactNumber: editdata.contactNumber,
          total: editdata.total, discount: editdata.discount, netTotal: editdata.netTotal,  percentageControl: editdata.percentageControl, details: this.editinvdetail
        })
      }
    });
  }
  ...
  SaveInvoice() {
    if (this.invoiceform.valid) {
      this.service.SaveInvoice(this.invoiceform.getRawValue()).subscribe(res => {
        ...
        if (result.id) {
          if(this.isedit){
            this.alert.success('Updated Successfully.', 'Invoice :' + result.id);
          }else{
          this.alert.success('Created Successfully.', 'Invoice :' + result.id);
          }
  ...
  productchange(event: any,index: any) {
    ...
    if(selectedProduct){
      this.service.GetProductbyId(selectedProduct).subscribe(res => {
        ...
        proddata = res;
        if (proddata != null) {
          this.invoiceproduct.get("productName")?.setValue(proddata.productName);
          this.invoiceproduct.get("description")?.setValue(proddata.description);
  ...
  summarycalculation() {
    ...
    let percentageControl = this.invoiceform.get("percentageControl")?.value ?? 0;
